...
contract AllowanceContract is ReentrancyGuard {
    ...
    function addTask(string memory _description) public onlyOwner {
        taskCounter++;
        tasks[taskCounter] = Task(_description, false, false);
    ...
    function payAllowance() public onlyOwner nonReentrant {
        ...
        uint256 totalPayment = totalApprovedTasks * allowancePerTask;
        require(usdcToken.balanceOf(address(this)) >= totalPayment, "Insufficient token balance");
        usdcToken.transfer(child, totalPayment);
        emit AllowancePaid(totalPayment);
    }
