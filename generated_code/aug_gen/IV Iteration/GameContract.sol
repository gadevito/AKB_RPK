...
contract GameContract is VRFConsumerBase {
    ...
    function createItem(string memory name, uint256 price) external {
        ...
        items.push(Item(name, price, 0, address(0)));
        requestIdToItemId[requestId] = itemId;
        emit ItemCreated(itemId, name, price, 0);
    }
    ...
    function transferItem(uint256 itemId, address recipient) external {
        require(players[msg.sender].registered, "Only registered players can transfer items");
    ...
    function addFunds(uint256 amount) external {
        require(players[msg.sender].registered, "Only registered players can add funds");
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        require(itemId < items.length, "Invalid requestId");
        items[itemId].rarity = randomResult;
        emit ItemCreated(itemId, items[itemId].name, items[itemId].price, randomResult);
    }
}