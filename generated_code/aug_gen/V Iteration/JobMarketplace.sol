...
contract JobMarketplace {
    ...
    function postJob(string memory _title, string memory _description) public payable {
        ...
        jobCount++;
        jobs[jobCount] = Job(jobCount, _title, _description, msg.value, msg.sender, address(0), Status.Open);

        emit JobPosted(jobCount, _title, _description, msg.value, msg.sender);
    }

    function bidOnJob(uint _jobId) public {
        require(_jobId > 0 && _jobId <= jobCount, "Invalid job ID");
        Job storage job = jobs[_jobId];
        require(job.client != msg.sender, "Clients cannot bid on their own jobs");
    ...
    function markJobCompleted(uint _jobId) public {
        require(_jobId > 0 && _jobId <= jobCount, "Invalid job ID");
        Job storage job = jobs[_jobId];
        require(job.freelancer == msg.sender, "Only the freelancer can mark the job as completed");
    ...
    function cancelJob(uint _jobId) public {
        require(_jobId > 0 && _jobId <= jobCount, "Invalid job ID");
        Job storage job = jobs[_jobId];
        require(job.status == Status.Open || job.status == Status.InProgress, "Job cannot be canceled");

        if (msg.sender == job.client) {
            if (job.status == Status.InProgress) {
                payable(job.freelancer).transfer(job.payment / 2); // Partial payment to freelancer
                payable(job.client).transfer(job.payment / 2); // Partial refund to client
            } else {
                payable(job.client).transfer(job.payment); // Full refund to client
            }
        } else if (msg.sender == job.freelancer) {
            require(job.status == Status.Open, "Freelancer can only cancel open jobs");
            payable(job.client).transfer(job.payment); // Full refund to client
        } else {
            revert("Only the client or freelancer can cancel the job");
        }
