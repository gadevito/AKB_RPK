...
contract JobMarketplace {
    ...
    function cancelJob(uint _jobId) public {
        require(_jobId > 0 && _jobId <= jobCount, "Invalid job ID");
        Job storage job = jobs[_jobId];
        require(msg.sender == job.freelancer || msg.sender == job.client, "Only the freelancer or client can cancel the job");
        require(job.status == Status.Open || job.status == Status.InProgress, "Job cannot be canceled");

        if (job.status == Status.InProgress && msg.sender == job.client) {
            require(address(this).balance >= job.payment, "Contract does not have enough balance");
            payable(job.client).transfer(job.payment);
        } else if (job.status == Status.InProgress && msg.sender == job.freelancer) {
            require(address(this).balance >= job.payment, "Contract does not have enough balance");
            payable(job.client).transfer(job.payment);
        }
