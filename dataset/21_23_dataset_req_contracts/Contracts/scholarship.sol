...
contract ScholarshipContract {
    ...
    constructor(
        address _openCertsStoreAddress,
        address _alumniStoreAddress,
    ...
    function isCertificateIssued(
        bytes32 _blockchainCertificateHash
    ) private view returns (address payable _address) {
    ...
    function unlockScholarship(
        bytes32 _blockchainCertificateHash
    ) public returns (bool) {
        uint256 toBitDegree = tokenContract
            .balanceOf(address(this))
            .mul(bitDegreeFee)
            .div(100);
        uint256 toStudent = tokenContract.balanceOf(address(this)).sub(
            toBitDegree
        );
        address payable studentAddress = isCertificateIssued(
            _blockchainCertificateHash
        );
