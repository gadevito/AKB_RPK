...
contract Charity {
    ...
    function contribute() public payable {
        require(
            msg.value >= minAmount,
            "You try to send less than the minimum donation amount"
        ); // require that the donator is more or equal from the minimum donation
        require(block.timestamp < endDate, "Donation has expired"); // require that charity didn't expire
        if (
            contributions[msg.sender] == 0
        ) // if the donator had not already contibute to the charity
        {
    ...
    function refund() public {
        ...
        require(
            contributions[msg.sender] > 0,
            "You haven't contibute to the charity"
        ); // require that the sender contibuted to the charity
    ...
    function vote(uint index) public {
        ...
        require(
            spending.voters[msg.sender] == false,
            "You have already voted for this spending request"
        ); // require that the sender hadn't already vote
    ...
    function makePayment(uint index) public {
        ...
        require(
            spending.completed == false,
            "This spending request is already completed"
        ); // require that the selected spending request is not completed
        require(msg.sender == admin, "Only admin can make the payment"); // require that tne sender is the admin
        require(
            spending.numOfVoters > numOfContributors / 2,
            "Not enough contributors voted for the spending request"
        ); // require that more than 50% of the contributors voted
        require(getMoneyRaised() >= goal, "Charity didn't reach the goal yet"); // require that charity reached the goal
        require(
            getMoneyRaised() >= spending.amount,
            "Charity don't have enough amount"
        ); // require that charity have the amount
