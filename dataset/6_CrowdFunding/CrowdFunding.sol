...
contract CrowdFunding {
    ...
    event CampaignSuccessful(uint indexed campaignId, address campaignCreator);
    event CampaignUnsucceeded(uint indexed campaignId, address campaignCreator);
    ...
    function contribute(uint campaignId) public payable nonReentrant {
        ...
        // Validation check on status and end time
        require(campaign.status == STATUS.ACTIVE, "campaign not active");
        if (block.timestamp > campaign.endsAt) {
            campaign.status = STATUS.UNSUCCEEDED;
            emit CampaignUnsucceeded(campaignId, campaign.campaignCreator);
            return;
        }
        ...
        if (msg.value <= remainingFundsNeeded) {
            ...
            if (campaign.totalContributions >= campaign.goal) {
                ...
                emit CampaignSuccessful(campaignId, campaign.campaignCreator);
            }
        }
    ...
    function getLatestCampaigns() public view returns (Campaign[] memory) {
        ...
        Campaign[] memory latestCampaigns = new Campaign[](
            latestCampaignsCount
        );
