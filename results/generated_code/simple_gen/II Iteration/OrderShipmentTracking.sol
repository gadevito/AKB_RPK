...
contract OrderShipmentTracking {
    ...
    function getCompletedShipments(address customer) public view returns (uint256[] memory) {
        uint256 count = 0;
        ...
        uint256[] memory completedOrderIndices = new uint256[](count);
        uint256 index = 0;
        for (uint256 i = 0; i < orders[customer].length; i++) {
            if (orders[customer][i].isDelivered) {
                completedOrderIndices[index] = i;
                index++;
            }
        }

        return completedOrderIndices;
    }
}