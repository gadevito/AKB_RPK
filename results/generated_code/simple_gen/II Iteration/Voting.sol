...
contract Voting {
    ...
    function startVoting() public onlyAdmin {
        ...
        require(block.timestamp <= endTime, "Voting cannot start after the end time.");
        votingActive = true;
    }

    function endVoting() public onlyAdmin {
        require(votingActive, "Voting is not active.");
        require(block.timestamp >= startTime, "Voting cannot end before the start time.");
    ...
    function getResult() public view returns (bytes32 winner, uint256 winningVotes) {
        ...
        for (uint256 i = 0; i < choices.length; i++) {
            if (votes[choices[i]] > leadingVotes) {
                ...
                tie = false;
            } else if (votes[choices[i]] == leadingVotes) {
                tie = true;
            }
        }
