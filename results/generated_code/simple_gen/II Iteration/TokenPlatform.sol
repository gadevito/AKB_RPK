...
contract TokenPlatform is ERC1155, Pausable, Ownable {
    ...
    function uri(uint256 tokenId) public view override returns (string memory) {
        Batch storage batch = batches[tokenId / batchSize];
        return string(abi.encodePacked(baseURI, batch.metadataCIDs[tokenId % batchSize]));
    }
    ...
    function mintBatch(address[] memory _producers, string[] memory _metadataCIDs) external onlyOwner whenNotPaused {
        require(_producers.length == _metadataCIDs.length, "Producers and metadataCIDs length mismatch");
        ...
        batch.producers = _producers;
        batch.metadataCIDs = _metadataCIDs;
        batch.redeemed = new bool[](_producers.length);
    ...
    function redeemToken(uint256 batchId, uint256 tokenId) external whenNotPaused {
        Batch storage batch = batches[batchId];
        require(batch.producers[tokenId] == msg.sender, "Only the producer can redeem the token");
        require(!batch.redeemed[tokenId], "Token already redeemed");

        batch.redeemed[tokenId] = true;
    }
    ...
    function onERC1155BatchReceived(address operator, address from, uint256[] memory ids, uint256[] memory values, bytes memory data) public virtual returns (bytes4) {
        // Additional logic can be added here if needed
