...
contract JobMarketplace {
    ...
    mapping(uint => mapping(address => bool)) public jobBids;
    ...
    function bidOnJob(uint _jobId) public {
        ...
        require(job.client != msg.sender, "Client cannot bid on their own job");
        require(job.status == Status.Open, "Job is not open for bidding");

        jobBids[_jobId][msg.sender] = true;

        emit JobBid(_jobId, msg.sender);
    }

    function acceptBid(uint _jobId, address _freelancer) public {
        require(_jobId > 0 && _jobId <= jobCount, "Invalid job ID");
        Job storage job = jobs[_jobId];
        ...
        require(jobBids[_jobId][_freelancer], "No valid bid from this freelancer");
    ...
    function markJobCompleted(uint _jobId) public {
        ...
        require(address(this).balance >= job.payment, "Insufficient contract balance");
    ...
    function cancelJob(uint _jobId) public {
        require(_jobId > 0 && _jobId <= jobCount, "Invalid job ID");
        Job storage job = jobs[_jobId];
        require(job.client == msg.sender || job.freelancer == msg.sender, "Only the client or freelancer can cancel the job");
