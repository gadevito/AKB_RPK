...
contract GameContract is VRFConsumerBase {
    ...
    mapping(bytes32 => PendingItem) public pendingItems;
    ...
    function createItem(string memory name, uint256 price) external {
        ...
        pendingItems[requestId] = PendingItem(name, price, msg.sender);
    }
    ...
    function getPlayerDetails(address player) external view returns (uint256 level, uint256 experience, uint256 balance) {
        require(players[player].registered, "Player does not exist");
    ...
    function getPlayerBalance() external view returns (uint256) {
        require(players[msg.sender].registered, "Player does not exist");
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        PendingItem memory pendingItem = pendingItems[requestId];
        require(pendingItem.creator != address(0), "Pending item does not exist");
        uint256 itemId = items.length;
        items.push(Item(pendingItem.name, pendingItem.price, randomResult % 100, address(0)));
        emit ItemCreated(itemId, pendingItem.name, pendingItem.price, randomResult % 100);
        delete pendingItems[requestId];
    }
}