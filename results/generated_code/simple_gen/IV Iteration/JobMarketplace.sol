...
contract JobMarketplace {
    ...
    function postJob(string memory _title, string memory _description, uint _payment) public payable {
        ...
        require(msg.value == _payment, "Payment must be sent with the job posting");
    ...
    function cancelJob(uint _jobId) public {
        ...
        if (msg.sender == job.client) {
            require(job.status == Status.Open, "Client can only cancel open jobs");
        } else if (msg.sender == job.freelancer) {
            require(job.status == Status.InProgress, "Freelancer can only cancel in-progress jobs");
        } else {
